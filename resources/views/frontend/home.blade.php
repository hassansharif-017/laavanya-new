...
@section('title', __('A Dream online fabric store in Gurgaon- Laavanya'))
@php 
...
@section('meta-content')
	...
	@if(app('request')->input('thumbnail')) 
		<meta property="og:image" itemprop="image"  content="{{ app('request')->input('thumbnail') }}" />
		<meta name="twitter:image" content="{{ app('request')->input('thumbnail') }}">
...
<script type="text/javascript">
	function load_home_page_ajax() {
		
		$.ajax({
			type : 'GET',
			url: base_url + '/load_home_page_ajax',
			dataType : "HTML",
			success: function (response){
				//alert(response);
				$('#html_{{ $PageVariation['home_variation'] }}').html(response)
				$('.category-carousel').owlCarousel('refresh');
