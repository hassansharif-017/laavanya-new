							...
							<tbody>
								...
								<tr id="row_delete_{{ $row->id }}">
									...
									<td class="pro-name-w" data-title="{{ __('Product') }}:">
                                        ...
                                        <p data-title="{{ __('Sold By') }}:">Sold By: <small><a href="{{ route('frontend.stores', [$row->options->seller_id, str_slug($row->options->store_name)]) }}">{{ $row->options->store_name }}</a></small></p>
									</td>
									...
									<td class="text-center pro-quantity-w" data-title="{{ __('Quantity') }}:">
										<div class="btn-group pro-quantity">
                                            <input name="quantity" style="border-top-right-radius: 0;border-bottom-right-radius: 0;" id="quantity-{{ $row->rowId }}" type="number" class="txt_quantity" step="0.25" min="1" max="{{ $row->options->stock_qty }}" value="{{ $row->qty }}"><br>
                                            <a class="btn theme-btn btn_quantity_update" href="javascript:;" data-rowid="{{ $row->rowId }}" data-size="{{ $row->options->unit }}" data-stock="{{ $row->options->stock_qty }}" data-oldvalue="{{ $row->qty }}">Update</a>
                                        </div>
									</td>
