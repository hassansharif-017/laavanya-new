	...
	<header class="header">
		<!--Top Header-->
		<div class="top-header" style="background-color: #3FDFD0; border:0px;">
			<div class="container">
				<div class="row ">
					<div class="col-lg-4">
						...
						<form method="GET" action="{{ route('frontend.search') }}">
							<div class="search-card">
								...
								<div class="cat-select" >
									<select class="form-select" name="category" style="background-color: #F3ECDA !important;">
										<option value="">{{ __('All Categories') }}</option>
										@php echo CategoryListOption(); @endphp
									</select>
								</div>
								<div class="search-btn">
									<button type="submit" class="btn btn-search"><i class="bi bi-search text-dark"></i></button>
								</div>
							</div>
						</form>
					</div>
				...
				<div class="row">
					<div class="col-lg-3">
						<ul class="categories-wrap">
							<li>
								<a class="navCategoryListActive" href="javascript:void(0);">{{ __('Browse Fabrics') }}</a>
								<ul class="nav-category-list" style=" overflow-y: scroll; height: 461px;">
									@php echo CategoryMenuList(); @endphp
									<li><a href="javascript:void(0);" class="btn cat-more-btn catMoreBtnActive"><span class="onCatMoreBtn">{{ __('') }}</span></a></li>
								</ul>
							</li>
						</ul>
					</div>
	...
	<aside class="mobile-menu-wrapper">
		...
		<div class="offcanvas-body">
			...
			<div class="search-for-mobile">
				<form method="GET" action="{{ route('frontend.search') }}">
					<input name="search" type="text" class="form-control" placeholder="{{ __('Search for Products') }}..." required />
					<button  type="submit" class="btn theme-btn"><i  class="bi bi-search text-dark"></i>{{ __('Search') }}</button>
				</form>	
			</div>
			<div class="mobile-navigation">
				<nav>
					<ul class="mobile-menu">
						<li class="has-children-menu"><a href="#">{{ __('Browse Fabrics') }}</a>
