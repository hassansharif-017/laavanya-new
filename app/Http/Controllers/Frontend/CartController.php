...
class CartController extends Controller
{
	...
	public function AddToCart($id, $qty){
		...
		$data['options']['stock_qty'] = $datalist['stock_qty'];
		...
		if($response){
            ...
            $res['msgType'] = 'error';
			$res['msg'] = __('Product is already in your cart.');
        }else{
			$res['msgType'] = 'error';
	...
	public function updateQuantity($qty,$rowid)
    {
        $product = Cart::instance('shopping')->search(function($cartItem, $rowId) use($rowid){
            return $rowId == $rowid;
        });
        $product_info = Product::where('id', $product[$rowid]->id)->first();
        if($qty <= $product_info->stock_qty){
            Cart::instance('shopping')->update($rowid,$qty);
			$res['msgType'] = 'success';
			$res['msg'] = __('Product quantity update Successfully');
		}else{
			$res['msgType'] = 'error';
			$res['msg'] = __('Product quantity updated failed.');
		}
        return response()->json($res);
    }
