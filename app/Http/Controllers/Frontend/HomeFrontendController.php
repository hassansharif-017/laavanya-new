...
class HomeFrontendController extends Controller
{
	...
	{
		$home_video = Tp_option::all();
        return view('frontend.aboutus', compact("home_video"));
	}

	public function offerad()
    {
        $Offerad = Offer_ad::all();
        return view('frontend.offer', compact("Offerad"));
    }
	...
	public function newarrival()
    {
        ...
        return view('frontend.newarrival', compact("new_products"));
    }

    public function homePageLoad____old(Request $request)
	{
		$lan = glan();
	...
	public function homePageLoad(Request $request)
	{
        ...
        //Home Page Section 1
        $data = [];
        $sections = Section_manage::where(['manage_type' => $PageVariation['home_variation'],'is_publish'=>1])->get();
        if(!empty($sections)){
            foreach($sections as $key => $value){
                $data['section'.($key+1)] = $value;
            }
        }
        //dd($data);
        //Slider
        ...
        //Offer & Ads - Position 1 (For Homepage 1)
        $data['offer_ad_position1'] = Offer_ad::where('is_publish', '=', 1)->where('offer_ad_type', '=', 'position1_home1')->orderBy('id', 'desc')->get();
        //Offer & Ads - Position 2 (For Homepage 1)
        $data['offer_ad_position2'] = Offer_ad::where('is_publish', '=', 1)->where('offer_ad_type', '=', 'position2_home1')->orderBy('id', 'desc')->offset(0)->limit(1)->get();
        ...
        if($hv_data ){
            $hvData = json_decode($hv_data);
            $dataObj = json_decode($hvData[0]->option_value);
            $data['home_video']['title'] = $dataObj->title;
            $data['home_video']['short_desc'] = $dataObj->short_desc;
            $data['home_video']['url'] = $dataObj->url;
            $data['home_video']['video_url'] = $dataObj->video_url;
            $data['home_video']['button_text'] = $dataObj->button_text;
            $data['home_video']['target'] = $dataObj->target;
            $data['home_video']['image'] = $dataObj->image;
            $data['home_video']['is_publish'] = $dataObj->is_publish;
        }else{
            $data['home_video']['title'] = '';
            $data['home_video']['short_desc'] = '';
            $data['home_video']['url'] = '';
            $data['home_video']['video_url'] = '';
            $data['home_video']['button_text'] = '';
            $data['home_video']['target'] = '';
            $data['home_video']['image'] = '';
            $data['home_video']['is_publish'] = '2';
        }

        //Brand
        $data['brand'] = Brand::where('is_publish', '=', 1)->where('is_featured', '=', 1)->where('lan', '=', $lan)->orderBy('id', 'desc')->get();
        ...
        ORDER BY TotalReview DESC
        LIMIT 8;";
        $data['top_rated'] = DB::select(DB::raw($tr_sql));
        ...
        ORDER BY a.id DESC;";
        $deals_products = DB::select(DB::raw($dofd_sql));
        $data['deals_products'] = $deals_products;
		session(['subscribePopupOff' => 'OFF']);
        return view('frontend.home')->with($data);
    }

	public function load_home_page_ajax(){
		$data = [];
		$lan = glan();
		$PageVariation = PageVariation();
		$sections = Section_manage::where(['manage_type' => $PageVariation['home_variation'],'is_publish'=>1])->get();
		...
		//exit;;
		return view('frontend.partials.ajaxloading')->with($data);
	}
}
