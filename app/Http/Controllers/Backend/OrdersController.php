...
class OrdersController extends Controller
{
	public function shiprocket($id) {        
		$ordermasterdata    = Order_master::where('id', $id )->get();
		$orderlistdata      = Order_item::where('order_master_id', $id )->get();
		$orderlistcount     = Order_item::where('order_master_id', $id )->count();   
		$orderDetails 		= Order_master::where('id',$id)->first()->toArray();
		//dd($orderDetails); die;
		//$tax = $orderDetails->tax;
		//$shipping_fee = $orderDetails->shipping_fee;
		$total_amount = $orderDetails['shipping_fee'];
		$shiprocket_order['order_id']=$orderDetails['id'];
		$shiprocket_order['order_date']=$orderDetails['created_at'];
		$shiprocket_order['pickup_location']="Laavanya";
		$shiprocket_order['channel_id']="1942180";
		$shiprocket_order['comment']="Order";
		$shiprocket_order['billing_customer_name']=$orderDetails['name'];
		$shiprocket_order['billing_last_name']=$orderDetails['name'];
		$shiprocket_order['billing_address']=$orderDetails['address'];
		$shiprocket_order['billing_address_2']="";
		$shiprocket_order['billing_city']=$orderDetails['city'];
		$shiprocket_order['billing_pincode']=$orderDetails['zip_code'];
		$shiprocket_order['billing_state']=$orderDetails['state'];
		$shiprocket_order['billing_country']=$orderDetails['country'];
		$shiprocket_order['billing_email']=$orderDetails['email'];
		$shiprocket_order['billing_phone']=$orderDetails['phone'];
		$shiprocket_order['shipping_is_billing']=1;
		$shiprocket_order['shipping_customer_name']=$orderDetails['name'];
		$shiprocket_order['shipping_last_name']=$orderDetails['name'];
		$shiprocket_order['shipping_address']=$orderDetails['address'];
		$shiprocket_order['shipping_address_2']="";
		$shiprocket_order['shipping_city']=$orderDetails['city'];
		$shiprocket_order['shipping_pincode']=$orderDetails['zip_code'];
		$shiprocket_order['shipping_state']=$orderDetails['state'];
		$shiprocket_order['shipping_country']=$orderDetails['country'];
		$shiprocket_order['shipping_email']=$orderDetails['email'];
		$shiprocket_order['shipping_phone']=$orderDetails['phone'];
		foreach ($orderlistdata as $key=>$item){
			$total_amount = $total_amount+($item->total_price+$item->tax);
			$shiprocket_order['order_items'][$key]['name'] = Product::where('id', $item['product_id'] )->value('title');
			$shiprocket_order['order_items'][$key]['sku'] = "SKU".$item['product_id'];
			$shiprocket_order['order_items'][$key]['units'] = $item['quantity'];
			$shiprocket_order['order_items'][$key]['selling_price'] = $item['total_price'];
			$shiprocket_order['order_items'][$key]['discount'] = "0";
			$shiprocket_order['order_items'][$key]['tax'] = "0";
			$shiprocket_order['order_items'][$key]['hsn'] = "";
		}
		$shiprocket_order['payment_method'] = ($orderDetails['payment_method_id']==1)?'COD':'PREPAID';
		$shiprocket_order['shipping_charges'] = "";
		$shiprocket_order['giftwrap_charges'] = "";
		$shiprocket_order['transaction_charges'] = "";
		$shiprocket_order['total_discount'] = "";
		$shiprocket_order['sub_total'] = $total_amount;
		$shiprocket_order['length'] = 29;
		$shiprocket_order['breadth'] = 1.5;
		$shiprocket_order['height'] = 3;
		$shiprocket_order['weight'] = 23;
		
		//echo "<pre>"; print_r($shiprocket_order); exit;
		//echo "<pre>"; print_r(json_encode($shiprocket_order)); exit;
		$shiprocket_order = json_encode($shiprocket_order,true);
		//Generate Token
		$c = curl_init();
		$url = "https://apiv2.shiprocket.in/v1/external/auth/login";
		curl_setopt($c, CURLOPT_URL, $url);
		curl_setopt($c, CURLOPT_POST, 1);
		curl_setopt($c, CURLOPT_POSTFIELDS, "email=laavanyayt@gmail.com&password=laavanya@123");
		curl_setopt($c, CURLOPT_RETURNTRANSFER, 1);
		$server_output = curl_exec($c);
		curl_close($c);
		$server_output = json_decode($server_output,true);
		// echo "<pre>"; print_r($server_output); die;
		//Create ordder in shiprocket
		$url = "https://apiv2.shiprocket.in/v1/external/orders/create/adhoc";
		$c = curl_init($url);
		curl_setopt($c, CURLOPT_POST, 1);
		curl_setopt($c, CURLOPT_POSTFIELDS, $shiprocket_order);
		curl_setopt($c, CURLOPT_RETURNTRANSFER, 1);
